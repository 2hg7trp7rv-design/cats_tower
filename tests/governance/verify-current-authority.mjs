...
if (s02ReferenceQaCorrection) {
  ...
  }), 'S02 review-reference correction does not bind the successful exact-entry authority workflow');
  assert(JSON.stringify(s02ReferenceQaCorrection.failedWorkflow) === JSON.stringify({
    commit: s02ReferenceQaCorrection.entry.head,
    tree: s02ReferenceQaCorrection.entry.tree,
    runId: 33591754259,
    runAttempt: 1,
    ...
    conclusion: 'FAILURE',
    failedStep: 'Capture and verify eight masters plus required responsive variants',
    error: 'Error: GM02: review reference did not load in ordinary review chrome',
    artifactCount: 0
  }), 'S02 review-reference correction does not bind the exact failed workflow/job/step/error');
  assert(JSON.stringify(s02ReferenceQaCorrection.diagnosis) === JSON.stringify({
    type: 'QA_REVIEW_REFERENCE_SCOPE_CONTRADICTION_AFTER_SUCCESSFUL_BROWSER_CAPTURE',
    collectorContract: 'ONE_VISIBLE_DECODED_REVIEW_REFERENCE_REQUIRED_IN_EVERY_SCENARIO',
    validatorContract: 'REJECTED_EVERY_PRIMARY_SCENARIO_WITH_REFERENCE_EXCEPT_GM01',
  ...
  assert(s02ReferenceQaCorrectionCommit && git(['rev-parse', `${s02ReferenceQaCorrectionCommit}^`]) === s02ReferenceQaCorrection.entry.head, 'S02 review-reference correction is not the immediate child of the failed commit');
  assert(git(['rev-parse', `${s02ReferenceQaCorrection.entry.head}^{tree}`]) === s02ReferenceQaCorrection.entry.tree, 'S02 review-reference correction entry tree mismatch');
  assertExactChangedPaths(s02ReferenceQaCorrection.entry.head, s02ReferenceQaCorrectionCommit, s02ReferenceQaCorrectionChangedPaths, 'S02 review-reference correction commit');
  assertAddedOnceAndUnchanged(s02ReferenceQaCorrectionPath, s02ReferenceQaCorrectionCommit);

  const priorReferenceQaSource = textAt(s02ReferenceQaCorrection.entry.head, 'tests/step4/s02-golden-master-p1-browser-qa.mjs');
  const correctedReferenceQaSource = textAt(s02ReferenceQaCorrectionCommit, 'tests/step4/s02-golden-master-p1-browser-qa.mjs');
  const contradictoryReferenceAssertion = "invariant(scenario.id === 'GM01' && scenario.diagnostics.resourcePaths.includes('/step4/s02/golden-master-p1/' + reference.path), scenario.id + ': review reference did not load in ordinary review chrome');";
  const everyScenarioReferenceAssertion = "invariant(scenario.diagnostics.resourcePaths.includes('/step4/s02/golden-master-p1/' + reference.path), scenario.id + ': review reference did not load in ordinary review chrome');";
  assert(priorReferenceQaSource.split(contradictoryReferenceAssertion).length === 2 && correctedReferenceQaSource === priorReferenceQaSource.replace(contradictoryReferenceAssertion, everyScenarioReferenceAssertion) && sha256Text(correctedReferenceQaSource) === s02ReferenceQaCorrection.correction.qaAfterSha256, 'S02 review-reference correction changed more than the contradictory scenario gate');
  const referenceQaPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02ReferenceQaCorrection.entry.head, s02ReferenceQaCorrectionCommit, '--', 'tests/step4/s02-golden-master-p1-browser-qa.mjs'], { cwd: root, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 });
  assert(`sha256:${sha256Text(referenceQaPatch)}` === s02ReferenceQaCorrection.correction.qaPatchSha256, 'S02 review-reference QA patch differs from the immutable correction record');

  const priorReferenceQaPin = '5190fe2fca12b75356518a02c95dae6947ac2f6e68690c43ac30d7ee82bf293a  tests/step4/s02-golden-master-p1-browser-qa.mjs';
  const correctedReferenceQaPin = '0778dd5e74031aab8a6db3ff44c0563aa5907c362bdad64779db781422ca5463  tests/step4/s02-golden-master-p1-browser-qa.mjs';
  const uploadStepName = '      - name: Upload uncredentialed semantic evidence\n';
  const uploadAlways = '        if: always()\n';
  const priorReferenceWorkflowSource = textAt(s02ReferenceQaCorrection.entry.head, '.github/workflows/verify-step-4-s02-golden-master-p1.yml');
  const correctedReferenceWorkflowSource = textAt(s02ReferenceQaCorrectionCommit, '.github/workflows/verify-step-4-s02-golden-master-p1.yml');
  assert(priorReferenceWorkflowSource.split(priorReferenceQaPin).length === 2 && priorReferenceWorkflowSource.split(uploadStepName).length === 2 && !priorReferenceWorkflowSource.includes(uploadStepName + uploadAlways), 'S02 review-reference prior workflow lacks the exact pin/upload marker');
  const expectedReferenceWorkflowSource = priorReferenceWorkflowSource.replace(priorReferenceQaPin, correctedReferenceQaPin).replace(uploadStepName, uploadStepName + uploadAlways);
  assert(correctedReferenceWorkflowSource === expectedReferenceWorkflowSource && sha256Text(correctedReferenceWorkflowSource) === s02ReferenceQaCorrection.correction.workflowAfterSha256, 'S02 review-reference workflow changed beyond the exact QA pin and always-upload diagnostic');
  const referenceWorkflowPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02ReferenceQaCorrection.entry.head, s02ReferenceQaCorrectionCommit, '--', '.github/workflows/verify-step-4-s02-golden-master-p1.yml'], { cwd: root, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 });
  assert(`sha256:${sha256Text(referenceWorkflowPatch)}` === s02ReferenceQaCorrection.correction.workflowPatchSha256, 'S02 review-reference workflow patch differs from the immutable correction record');

  const correctedReferenceVerifierSource = textAt(s02ReferenceQaCorrectionCommit, 'tests/governance/verify-current-authority.mjs');
  ...
  if (requireLiveActions) {
    ...
    const authorityArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02ReferenceQaCorrection.authorityWorkflow.artifactId}`);
    assert(authorityRun.head_sha === s02ReferenceQaCorrection.entry.head && authorityRun.head_branch === 'kimi' && authorityRun.status === 'completed' && authorityRun.conclusion === 'success' && authorityRun.run_attempt === 1, 'S02 review-reference correction live authority run mismatch');
    assert(authorityJob.run_id === authorityRun.id && authorityJob.head_sha === s02ReferenceQaCorrection.entry.head && authorityJob.name === 'current-authority' && authorityJob.conclusion === 'success', 'S02 review-reference correction live authority job mismatch');
    assert(authorityArtifact.name === s02ReferenceQaCorrection.authorityWorkflow.artifactName && authorityArtifact.digest === s02ReferenceQaCorrection.authorityWorkflow.artifactDigest && authorityArtifact.expired === false && authorityArtifact.workflow_run?.head_sha === s02ReferenceQaCorrection.entry.head, 'S02 review-reference correction live authority artifact mismatch');
    const failedRun = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/runs/${s02ReferenceQaCorrection.failedWorkflow.runId}`);
    const failedJob = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/jobs/${s02ReferenceQaCorrection.failedWorkflow.jobId}`);
    assert(failedRun.head_sha === s02ReferenceQaCorrection.entry.head && failedRun.head_branch === 'kimi' && failedRun.status === 'completed' && failedRun.conclusion === 'failure' && failedRun.run_attempt === 1, 'S02 review-reference correction live failed run mismatch');
    assert(failedJob.run_id === failedRun.id && failedJob.head_sha === s02ReferenceQaCorrection.entry.head && failedJob.name === 'Static, eight-master responsive and accessibility verification' && failedJob.conclusion === 'failure', 'S02 review-reference correction live failed job mismatch');
    const failedSteps = (failedJob.steps ?? []).filter(step => step.name === s02ReferenceQaCorrection.failedWorkflow.failedStep);
    assert(failedSteps.length === 1 && failedSteps[0].conclusion === 'failure', 'S02 review-reference correction live failed step mismatch');
  }
}
...
if (s02BrowserEvidenceCorrection) {
  ...
  assert(s02BrowserEvidenceCorrection.correction.workflowBeforeSha256 === expectedS02WorkflowCorrectionRound003Sha256 && s02BrowserEvidenceCorrection.correction.workflowAfterSha256 === expectedS02WorkflowCorrectionRound004Sha256 && s02BrowserEvidenceCorrection.correction.workflowPatchSha256 === 'sha256:845468c1396c380880ea7a5708f0fd71a486781101fa0f35cc5ad180ad32549c', 'S02 browser-evidence workflow digest/patch mismatch');
  ...
  assert(priorWorkflowSource.split(priorBrowserPin).length === 2 && priorWorkflowSource.split(priorQaPin).length === 2 && correctedWorkflowSource === expectedWorkflowSource && sha256Text(correctedWorkflowSource) === expectedS02WorkflowCorrectionRound004Sha256, 'S02 browser-evidence workflow changed beyond the exact collector and QA pins');
  ...
  const fourthCorrectionFreezeEnd = s02ExactCaptureCorrectionCommit ? git(['rev-parse', `${s02ExactCaptureCorrectionCommit}^`]) : git(['rev-parse', 'HEAD']);
  assertNoPathChangesSince(s02BrowserEvidenceCorrectionCommit, fourthCorrectionFreezeEnd, s02BrowserEvidenceCorrectionChangedPaths, 'S02 browser-evidence correction freeze before exact-capture correction');
  if (s02ExactCaptureCorrectionCommit) assertNoPathChangesSince(s02BrowserEvidenceCorrectionCommit, 'HEAD', [s02BrowserEvidenceCorrectionPath], 'S02 browser-evidence round 004 record freeze');
  if (requireLiveActions) {
...
if (s02ExactCaptureCorrection) {
  assertExactKeySet(s02ExactCaptureCorrection, ['schemaVersion', 'artifactId', 'createdAt', 'repository', 'branch', 'changeControl', 'entry', 'authorityWorkflow', 'failedWorkflow', 'diagnosis', 'correction', 'boundaries', 'status'], 'S02 exact-capture correction');
  assertExactKeySet(s02ExactCaptureCorrection.entry, ['head', 'tree'], 'S02 exact-capture correction entry');
  assertExactKeySet(s02ExactCaptureCorrection.authorityWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 exact-capture authority workflow');
  assertExactKeySet(s02ExactCaptureCorrection.failedWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'failedStep', 'error', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 exact-capture failed workflow');
  assertExactKeySet(s02ExactCaptureCorrection.diagnosis, ['type', 'firstExactCaptureSucceeded', 'failedCapturePosition', 'rootCause', 'originalFailureArtifactPreserved'], 'S02 exact-capture diagnosis');
  assertExactKeySet(s02ExactCaptureCorrection.correction, ['rule', 'changedPaths', 'workflowBeforeSha256', 'workflowAfterSha256', 'workflowPatchSha256', 'browserBeforeSha256', 'browserAfterSha256', 'browserPatchSha256', 'verifierAfterSha256', 'verifierPatchSha256', 'acceptanceThresholdsChanged', 'qualityCoverageWeakened', 'goldenMasterLayoutChanged', 'captureOriginAsserted', 'failureEvidenceUploadRetained'], 'S02 exact-capture exact correction');
  assertExactKeySet(s02ExactCaptureCorrection.boundaries, ['rootRuntimeChanged', 'gameCoreChanged', 'gameDataChanged', 'economyChanged', 'saveSchemaChanged', 'backendChanged', 'productionChanged', 'productionAliasChanged', 'physicalIPhoneVerified', 'step4Pass', 'step5Allowed'], 'S02 exact-capture correction boundaries');
  assert(s02ExactCaptureCorrection.schemaVersion === 1 && s02ExactCaptureCorrection.artifactId === 'cats-tower-s02-golden-master-p1-trusted-harness-correction-round-005' && s02ExactCaptureCorrection.createdAt === '2026-09-02', 'S02 exact-capture correction identity/date mismatch');
  assert(s02ExactCaptureCorrection.repository === '2hg7trp7rv-design/cats_tower' && s02ExactCaptureCorrection.branch === 'kimi' && s02ExactCaptureCorrection.changeControl === s02RepairControlPath, 'S02 exact-capture correction authority mismatch');
  assert(JSON.stringify(s02ExactCaptureCorrection.entry) === JSON.stringify({ head: 'f54514be7ddd1b0d46b70773ed3a9b7b121b8688', tree: '612edc4b149e51ff66cd05a3c573af5685e7ae91' }), 'S02 exact-capture correction entry is not the exact failed browser-evidence correction commit/tree');
  assert(JSON.stringify(s02ExactCaptureCorrection.authorityWorkflow) === JSON.stringify({
    commit: s02ExactCaptureCorrection.entry.head,
    tree: s02ExactCaptureCorrection.entry.tree,
    runId: 33594278913,
    runAttempt: 1,
    jobId: 100134589312,
    conclusion: 'SUCCESS',
    artifactId: 9832852102,
    artifactName: 'phase0-current-governance-f54514be7ddd1b0d46b70773ed3a9b7b121b8688-33594278913-1',
    artifactDigest: 'sha256:96b2e3e4d717391257efaa52ca9fea478e09d0be943cc4b56962618b2b48cc8c'
  }), 'S02 exact-capture correction does not bind the successful exact-entry authority workflow');
  assert(JSON.stringify(s02ExactCaptureCorrection.failedWorkflow) === JSON.stringify({
    commit: s02ExactCaptureCorrection.entry.head,
    tree: s02ExactCaptureCorrection.entry.tree,
    runId: 33594278948,
    runAttempt: 1,
    ...
    artifactId: 9832840162,
    artifactName: 'step4-s02-golden-master-p1-semantic-f54514be7ddd1b0d46b70773ed3a9b7b121b8688-33594278948-1',
    artifactDigest: 'sha256:4af8242aeab6aeee266e98e4d2e01e2c742661b983c338efe123a1271f2fed48'
  }), 'S02 exact-capture correction does not bind the exact failed workflow/job/error/artifact');
  assert(JSON.stringify(s02ExactCaptureCorrection.diagnosis) === JSON.stringify({
    type: 'PAGE_SCREENSHOT_CLIP_COORDINATE_SPACE_OUTSIDE_VIEWPORT',
    firstExactCaptureSucceeded: 'GM01-390x844.png',
    failedCapturePosition: 'The next 320x667 stage retained review-page offset while the page clip was bounded by the 320x667 viewport.',
    rootCause: 'The exact-dimension repair used ordinary review-page element coordinates for a viewport-sized page screenshot without first normalizing the Golden Master stage to the screenshot origin.',
  ...
  assert(s02ExactCaptureCorrection.status === 'CORRECTED_EXACT_CAPTURE_ORIGIN_PENDING_RERUN', 'S02 exact-capture correction status mismatch');
  assert(s02ExactCaptureCorrectionCommit && git(['rev-parse', `${s02ExactCaptureCorrectionCommit}^`]) === s02ExactCaptureCorrection.entry.head, 'S02 exact-capture correction is not the immediate child of the exact failed commit');
  assert(git(['rev-parse', `${s02ExactCaptureCorrection.entry.head}^{tree}`]) === s02ExactCaptureCorrection.entry.tree, 'S02 exact-capture correction entry tree mismatch');
  assertExactChangedPaths(s02ExactCaptureCorrection.entry.head, s02ExactCaptureCorrectionCommit, s02ExactCaptureCorrectionChangedPaths, 'S02 exact-capture correction commit');
  assertAddedOnceAndUnchanged(s02ExactCaptureCorrectionPath, s02ExactCaptureCorrectionCommit);

  const priorExactCaptureBrowserSource = textAt(s02ExactCaptureCorrection.entry.head, 'tests/step4/s02-golden-master-p1-browser.mjs');
  const correctedExactCaptureBrowserSource = textAt(s02ExactCaptureCorrectionCommit, 'tests/step4/s02-golden-master-p1-browser.mjs');
  const exactCaptureBrowserPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02ExactCaptureCorrection.entry.head, s02ExactCaptureCorrectionCommit, '--', 'tests/step4/s02-golden-master-p1-browser.mjs'], { cwd: root, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 });
  assert(sha256Text(priorExactCaptureBrowserSource) === s02ExactCaptureCorrection.correction.browserBeforeSha256 && sha256Text(correctedExactCaptureBrowserSource) === s02ExactCaptureCorrection.correction.browserAfterSha256 && `sha256:${sha256Text(exactCaptureBrowserPatch)}` === s02ExactCaptureCorrection.correction.browserPatchSha256, 'S02 exact-capture collector bytes/patch differ from the immutable correction record');
  assert(correctedExactCaptureBrowserSource.split('const exactStageCaptureCss = ').length === 2 && correctedExactCaptureBrowserSource.split('async function captureExactStage(page, exactStage, scenario)').length === 2 && correctedExactCaptureBrowserSource.split('const bytes = await captureExactStage(page, exactStage, scenario);').length === 3, 'S02 exact-capture helper is absent, duplicated or not used for both current and baseline captures');
  assert(correctedExactCaptureBrowserSource.includes("clip: { x: 0, y: 0, width: scenario.width, height: scenario.height }") && correctedExactCaptureBrowserSource.includes('captureBeyondViewport: false') && !correctedExactCaptureBrowserSource.includes('Math.round(stageBox.x)') && !correctedExactCaptureBrowserSource.includes('Math.round(stageBox.y)'), 'S02 exact-capture collector retains the failed coordinate-space capture path');
  assert(correctedExactCaptureBrowserSource.indexOf('const collected = await collectDom(') < correctedExactCaptureBrowserSource.indexOf('const bytes = await captureExactStage(page, exactStage, scenario);'), 'S02 exact-capture normalization occurs before semantic layout measurement');

  const priorExactCaptureWorkflowSource = textAt(s02ExactCaptureCorrection.entry.head, '.github/workflows/verify-step-4-s02-golden-master-p1.yml');
  ...
  if (requireLiveActions) {
    ...
    const authorityArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02ExactCaptureCorrection.authorityWorkflow.artifactId}`);
    assert(authorityRun.head_sha === s02ExactCaptureCorrection.entry.head && authorityRun.head_branch === 'kimi' && authorityRun.status === 'completed' && authorityRun.conclusion === 'success' && authorityRun.run_attempt === 1, 'S02 exact-capture correction live authority run mismatch');
    assert(authorityJob.run_id === authorityRun.id && authorityJob.head_sha === s02ExactCaptureCorrection.entry.head && authorityJob.name === 'current-authority' && authorityJob.conclusion === 'success', 'S02 exact-capture correction live authority job mismatch');
    assert(authorityArtifact.name === s02ExactCaptureCorrection.authorityWorkflow.artifactName && authorityArtifact.digest === s02ExactCaptureCorrection.authorityWorkflow.artifactDigest && authorityArtifact.expired === false && authorityArtifact.workflow_run?.head_sha === s02ExactCaptureCorrection.entry.head, 'S02 exact-capture correction live authority artifact mismatch');
    const failedRun = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/runs/${s02ExactCaptureCorrection.failedWorkflow.runId}`);
    const failedJob = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/jobs/${s02ExactCaptureCorrection.failedWorkflow.jobId}`);
    const failedArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02ExactCaptureCorrection.failedWorkflow.artifactId}`);
    assert(failedRun.head_sha === s02ExactCaptureCorrection.entry.head && failedRun.head_branch === 'kimi' && failedRun.status === 'completed' && failedRun.conclusion === 'failure' && failedRun.run_attempt === 1, 'S02 exact-capture correction live failed run mismatch');
    assert(failedJob.run_id === failedRun.id && failedJob.head_sha === s02ExactCaptureCorrection.entry.head && failedJob.name === 'Static, eight-master responsive and accessibility verification' && failedJob.conclusion === 'failure', 'S02 exact-capture correction live failed job mismatch');
    const failedSteps = (failedJob.steps ?? []).filter(step => step.name === s02ExactCaptureCorrection.failedWorkflow.failedStep);
    assert(failedSteps.length === 1 && failedSteps[0].conclusion === 'failure', 'S02 exact-capture correction live failed step mismatch');
    assert(failedArtifact.name === s02ExactCaptureCorrection.failedWorkflow.artifactName && failedArtifact.digest === s02ExactCaptureCorrection.failedWorkflow.artifactDigest && failedArtifact.expired === false && failedArtifact.workflow_run?.head_sha === s02ExactCaptureCorrection.entry.head, 'S02 exact-capture correction live failure artifact mismatch');
  }
}
...
if (s02FourFindingCorrection) {
  ...
  const eighthCorrectionFreezeEnd = s02TwoFindingCorrectionCommit ? git(['rev-parse', `${s02TwoFindingCorrectionCommit}^`]) : git(['rev-parse', 'HEAD']);
  assertNoPathChangesSince(s02FourFindingCorrectionCommit, eighthCorrectionFreezeEnd, s02FourFindingCorrectionChangedPaths, 'S02 four-finding correction freeze before two-finding correction');
  if (s02TwoFindingCorrectionCommit) assertNoPathChangesSince(s02FourFindingCorrectionCommit, 'HEAD', [s02FourFindingCorrectionPath], 'S02 four-finding round 008 record freeze');
  if (requireLiveActions) {
...
// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_009
if (s02TwoFindingCorrection) {
  assertExactKeySet(s02TwoFindingCorrection, ['schemaVersion', 'artifactId', 'createdAt', 'repository', 'branch', 'changeControl', 'entry', 'authorityWorkflow', 'failedWorkflow', 'diagnosis', 'correction', 'boundaries', 'status'], 'S02 two-finding correction');
  assertExactKeySet(s02TwoFindingCorrection.entry, ['head', 'tree'], 'S02 two-finding correction entry');
  assertExactKeySet(s02TwoFindingCorrection.authorityWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 two-finding authority workflow');
  assertExactKeySet(s02TwoFindingCorrection.failedWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'failedStep', 'summary', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 two-finding failed workflow');
  assertExactKeySet(s02TwoFindingCorrection.diagnosis, ['type', 'failedAssertions', 'closedFindings', 'visualFindings', 'measurementFindings', 'allFifteenExactPngsPreserved', 'originalFailureArtifactPreserved'], 'S02 two-finding diagnosis');
  assertExactKeySet(s02TwoFindingCorrection.correction, ['rule', 'changedPaths', 'stylesBeforeSha256', 'stylesAfterSha256', 'stylesPatchSha256', 'manifestBeforeSha256', 'manifestAfterSha256', 'manifestPatchSha256', 'verifierAfterSha256', 'verifierPatchSha256', 'acceptanceThresholdsChanged', 'qualityCoverageWeakened', 'browserCollectorChanged', 'browserQaChanged', 'fontSizeReduced', 'failureEvidenceUploadRetained'], 'S02 two-finding exact correction');
  assertExactKeySet(s02TwoFindingCorrection.boundaries, ['rootRuntimeChanged', 'gameCoreChanged', 'gameDataChanged', 'economyChanged', 'saveSchemaChanged', 'backendChanged', 'productionChanged', 'productionAliasChanged', 'physicalIPhoneVerified', 'step4Pass', 'step5Allowed'], 'S02 two-finding correction boundaries');
  assert(s02TwoFindingCorrection.schemaVersion === 1 && s02TwoFindingCorrection.artifactId === 'cats-tower-s02-golden-master-p1-trusted-harness-correction-round-009' && s02TwoFindingCorrection.createdAt === '2026-09-02', 'S02 two-finding correction identity/date mismatch');
  assert(s02TwoFindingCorrection.repository === '2hg7trp7rv-design/cats_tower' && s02TwoFindingCorrection.branch === 'kimi' && s02TwoFindingCorrection.changeControl === s02RepairControlPath, 'S02 two-finding correction authority mismatch');
  assert(JSON.stringify(s02TwoFindingCorrection.entry) === JSON.stringify({ head: '29136e44d56842ab74f9d7660b4144d81e0eda41', tree: 'fd01bb2f3ee98ca50ff0a598e3996f36578a61a2' }), 'S02 two-finding correction entry is not the exact failed four-finding correction commit/tree');
  assert(JSON.stringify(s02TwoFindingCorrection.authorityWorkflow) === JSON.stringify({
    commit: s02TwoFindingCorrection.entry.head,
    tree: s02TwoFindingCorrection.entry.tree,
    runId: 33597789006,
    runAttempt: 1,
    jobId: 100144808622,
    conclusion: 'SUCCESS',
    artifactId: 9834026832,
    artifactName: 'phase0-current-governance-29136e44d56842ab74f9d7660b4144d81e0eda41-33597789006-1',
    artifactDigest: 'sha256:b4ee4b22aca6a153be5f6fc53919977fe91cbffe9e0619ca1421fbb0c17cddab'
  }), 'S02 two-finding correction does not bind the successful exact-entry authority workflow');
  assert(JSON.stringify(s02TwoFindingCorrection.failedWorkflow) === JSON.stringify({
    commit: s02TwoFindingCorrection.entry.head,
    tree: s02TwoFindingCorrection.entry.tree,
    runId: 33597789020,
    runAttempt: 1,
    ...
    conclusion: 'FAILURE',
    failedStep: 'Capture and verify eight masters plus required responsive variants',
    summary: 'FAIL_S02_GOLDEN_MASTER_P1_BROWSER: 15 scenarios, 2 failures',
    artifactId: 9834021016,
    artifactName: 'step4-s02-golden-master-p1-semantic-29136e44d56842ab74f9d7660b4144d81e0eda41-33597789020-1',
    artifactDigest: 'sha256:4f39120582ddc3a0864df6de91d520e830798fca9ec5cd5e600df1d906390119'
  }), 'S02 two-finding correction does not bind the exact failed workflow/job/summary/artifact');
  assert(JSON.stringify(s02TwoFindingCorrection.diagnosis) === JSON.stringify({
    type: 'ROUND_008_CLOSED_FLOOR_GEOMETRY_AND_TEXT200_OFFLINE_COLOR_BUT_EXPOSED_HEADER_AND_PRIMARY_REFLOW',
    failedAssertions: [
      'GM07C320: screenshot pixel complexity/opacity gate failed',
      'TEXT_200_PERCENT_NO_LOSS: trusted recomputation failed'
    ],
    closedFindings: [
      'RESPONSIVE_GEOMETRY_CONTRACT passed after the 200 percent floor line box reached its glyph scroll height.',
      'GM07C320TEXT200 reached 258 quantized colors and passed the unchanged 256 threshold.'
    ],
    visualFindings: [
      'GM07C320 reached 243 quantized colors and needed a second complementary material inlay rather than a threshold reduction.'
    ],
    measurementFindings: [
      'The three-column 200 percent header gave the objective only 82 CSS pixels, causing the objective and AUTO labels to overlap the enemy HP panel.',
      'The 28 CSS-pixel primary action label needed 98 CSS pixels of content height while the button exposed only 90 CSS pixels.'
    ],
    allFifteenExactPngsPreserved: true,
    originalFailureArtifactPreserved: true
  }), 'S02 two-finding diagnosis differs from the preserved fifteen-capture failed run and artifact');
  assert(s02TwoFindingCorrection.correction.rule === 'ADD_A_COMPLEMENTARY_BOTTOM_MATERIAL_INLAY_AND_REFLOW_THE_200_PERCENT_HEADER_TO_TWO_ROWS_WHILE_EXPANDING_THE_BATTLE_AND_PRIMARY_ROWS_WITHOUT_SHRINKING_TEXT_OR_LOWERING_THRESHOLDS' && JSON.stringify(s02TwoFindingCorrection.correction.changedPaths) === JSON.stringify(s02TwoFindingCorrectionChangedPaths), 'S02 two-finding correction rule/path set mismatch');
  assert(s02TwoFindingCorrection.correction.stylesBeforeSha256 === 'cb933fb01668a789052274ad7be87f075c114d013ee24b7fc7247d01bccd52e5' && s02TwoFindingCorrection.correction.stylesAfterSha256 === '666c48708f67d6e2b24d1861bae8bbf1778a4d5a34b3fd5d81a74bc429c8699b' && s02TwoFindingCorrection.correction.stylesPatchSha256 === 'sha256:1694b2bf31236928ddf5b6aa1d54fa1b80e49c33eaa165c95a48eede4b2f2102', 'S02 two-finding style digest/patch mismatch');
  assert(s02TwoFindingCorrection.correction.manifestBeforeSha256 === 'e08b4c46ced3259dfca6c4692d481cfbb6d4cda8e7065784a4763496b72e5335' && s02TwoFindingCorrection.correction.manifestAfterSha256 === '2307d9c35cfcff06d239bcbc3b3870b78ad1feff7d6afcf30c816001a46e3625' && s02TwoFindingCorrection.correction.manifestPatchSha256 === 'sha256:57054bad21d4d2dc4785e9f2ccd0865fb5ee5d41c66a774a0e519368c16b32b4', 'S02 two-finding manifest digest/patch mismatch');
  assert(s02TwoFindingCorrection.correction.acceptanceThresholdsChanged === false && s02TwoFindingCorrection.correction.qualityCoverageWeakened === false && s02TwoFindingCorrection.correction.browserCollectorChanged === false && s02TwoFindingCorrection.correction.browserQaChanged === false && s02TwoFindingCorrection.correction.fontSizeReduced === false && s02TwoFindingCorrection.correction.failureEvidenceUploadRetained === true, 'S02 two-finding correction weakens coverage, changes evidence collection or shrinks text');
  assert(JSON.stringify(s02TwoFindingCorrection.boundaries) === JSON.stringify({ rootRuntimeChanged: false, gameCoreChanged: false, gameDataChanged: false, economyChanged: false, saveSchemaChanged: false, backendChanged: false, productionChanged: false, productionAliasChanged: false, physicalIPhoneVerified: false, step4Pass: false, step5Allowed: false }), 'S02 two-finding correction crosses a protected product/release boundary');
  assert(s02TwoFindingCorrection.status === 'CORRECTED_TWO_BROWSER_FINDINGS_PENDING_RERUN', 'S02 two-finding correction status mismatch');
  assert(s02TwoFindingCorrectionCommit && git(['rev-parse', `${s02TwoFindingCorrectionCommit}^`]) === s02TwoFindingCorrection.entry.head, 'S02 two-finding correction is not the immediate child of the exact failed commit');
  assert(git(['rev-parse', `${s02TwoFindingCorrection.entry.head}^{tree}`]) === s02TwoFindingCorrection.entry.tree, 'S02 two-finding correction entry tree mismatch');
  assertExactChangedPaths(s02TwoFindingCorrection.entry.head, s02TwoFindingCorrectionCommit, s02TwoFindingCorrectionChangedPaths, 'S02 two-finding correction commit');
  assertAddedOnceAndUnchanged(s02TwoFindingCorrectionPath, s02TwoFindingCorrectionCommit);

  const priorTwoFindingStylesSource = textAt(s02TwoFindingCorrection.entry.head, 'step4/s02/golden-master-p1/styles.css');
  const correctedTwoFindingStylesSource = textAt(s02TwoFindingCorrectionCommit, 'step4/s02/golden-master-p1/styles.css');
  const twoFindingStylesPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02TwoFindingCorrection.entry.head, s02TwoFindingCorrectionCommit, '--', 'step4/s02/golden-master-p1/styles.css'], { cwd: root, encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
  assert(sha256Text(priorTwoFindingStylesSource) === s02TwoFindingCorrection.correction.stylesBeforeSha256 && sha256Text(correctedTwoFindingStylesSource) === s02TwoFindingCorrection.correction.stylesAfterSha256 && `sha256:${sha256Text(twoFindingStylesPatch)}` === s02TwoFindingCorrection.correction.stylesPatchSha256, 'S02 two-finding style bytes/patch differ from the immutable correction record');
  assert(correctedTwoFindingStylesSource.includes('background: linear-gradient(90deg, #75402a 0%, #b96d51 17%, #527f77 35%, #e5bd64 52%, #6c537d 69%, #4f8278 84%, #75402a 100%);') && correctedTwoFindingStylesSource.includes('grid-template-rows: 120px 56px;') && correctedTwoFindingStylesSource.includes('grid-column: 1 / -1;') && correctedTwoFindingStylesSource.includes('height: 100px;'), 'S02 two-finding correction lacks the complementary inlay or two-row 200-percent reflow');
  assert(priorTwoFindingStylesSource.split('font-size:').length === correctedTwoFindingStylesSource.split('font-size:').length, 'S02 two-finding correction shrinks or adds typography instead of reflowing it');

  const priorTwoFindingManifestSource = textAt(s02TwoFindingCorrection.entry.head, 'step4/s02/golden-master-p1/review-manifest.json');
  const correctedTwoFindingManifestSource = textAt(s02TwoFindingCorrectionCommit, 'step4/s02/golden-master-p1/review-manifest.json');
  const twoFindingManifestPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02TwoFindingCorrection.entry.head, s02TwoFindingCorrectionCommit, '--', 'step4/s02/golden-master-p1/review-manifest.json'], { cwd: root, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 });
  assert(sha256Text(priorTwoFindingManifestSource) === s02TwoFindingCorrection.correction.manifestBeforeSha256 && sha256Text(correctedTwoFindingManifestSource) === s02TwoFindingCorrection.correction.manifestAfterSha256 && `sha256:${sha256Text(twoFindingManifestPatch)}` === s02TwoFindingCorrection.correction.manifestPatchSha256, 'S02 two-finding manifest bytes/patch differ from the immutable correction record');
  const correctedTwoFindingManifest = JSON.parse(correctedTwoFindingManifestSource);
  const correctedTwoStylesManifestEntry = correctedTwoFindingManifest.routeFiles?.find(entry => entry.path === 'step4/s02/golden-master-p1/styles.css');
  assert(JSON.stringify(correctedTwoStylesManifestEntry) === JSON.stringify({ path: 'step4/s02/golden-master-p1/styles.css', bytes: 52958, sha256: s02TwoFindingCorrection.correction.stylesAfterSha256, gitBlob: '57f2a74e863e4d6d4166341477ecee8aff6ab54f' }), 'S02 two-finding manifest does not bind the corrected stylesheet bytes/hash/blob');

  const correctedTwoFindingVerifierSource = textAt(s02TwoFindingCorrectionCommit, 'tests/governance/verify-current-authority.mjs');
  const twoFindingVerifierPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02TwoFindingCorrection.entry.head, s02TwoFindingCorrectionCommit, '--', 'tests/governance/verify-current-authority.mjs'], { cwd: root, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
  assert(sha256Text(correctedTwoFindingVerifierSource) === s02TwoFindingCorrection.correction.verifierAfterSha256 && `sha256:${sha256Text(twoFindingVerifierPatch)}` === s02TwoFindingCorrection.correction.verifierPatchSha256, 'S02 two-finding verifier bytes/patch differ from the immutable correction record');
  assert(correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_001') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_002') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_003') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_004') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_005') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_006') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_007') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_008') && correctedTwoFindingVerifierSource.includes('// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_009') && correctedTwoFindingVerifierSource.includes("assertBoundary(paths, control, `${label} commit ${commit}`);"), 'S02 two-finding verifier removed a prior correction guard or original fail-closed boundary assertion');
  const s02TwoFindingFreezeEnd = s02RemainingTwoCorrectionCommit ? git(['rev-parse', `${s02RemainingTwoCorrectionCommit}^`]) : 'HEAD';
  assertNoPathChangesSince(s02TwoFindingCorrectionCommit, s02TwoFindingFreezeEnd, s02TwoFindingCorrectionChangedPaths, 'S02 two-finding correction freeze');
  if (requireLiveActions) {
    const authorityRun = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/runs/${s02TwoFindingCorrection.authorityWorkflow.runId}`);
    const authorityJob = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/jobs/${s02TwoFindingCorrection.authorityWorkflow.jobId}`);
    const authorityArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02TwoFindingCorrection.authorityWorkflow.artifactId}`);
    assert(authorityRun.head_sha === s02TwoFindingCorrection.entry.head && authorityRun.head_branch === 'kimi' && authorityRun.status === 'completed' && authorityRun.conclusion === 'success' && authorityRun.run_attempt === 1, 'S02 two-finding correction live authority run mismatch');
    assert(authorityJob.run_id === authorityRun.id && authorityJob.head_sha === s02TwoFindingCorrection.entry.head && authorityJob.name === 'current-authority' && authorityJob.conclusion === 'success', 'S02 two-finding correction live authority job mismatch');
    assert(authorityArtifact.name === s02TwoFindingCorrection.authorityWorkflow.artifactName && authorityArtifact.digest === s02TwoFindingCorrection.authorityWorkflow.artifactDigest && authorityArtifact.expired === false && authorityArtifact.workflow_run?.head_sha === s02TwoFindingCorrection.entry.head, 'S02 two-finding correction live authority artifact mismatch');
    const failedRun = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/runs/${s02TwoFindingCorrection.failedWorkflow.runId}`);
    const failedJob = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/jobs/${s02TwoFindingCorrection.failedWorkflow.jobId}`);
    const failedArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02TwoFindingCorrection.failedWorkflow.artifactId}`);
    assert(failedRun.head_sha === s02TwoFindingCorrection.entry.head && failedRun.head_branch === 'kimi' && failedRun.status === 'completed' && failedRun.conclusion === 'failure' && failedRun.run_attempt === 1, 'S02 two-finding correction live failed run mismatch');
    assert(failedJob.run_id === failedRun.id && failedJob.head_sha === s02TwoFindingCorrection.entry.head && failedJob.name === 'Static, eight-master responsive and accessibility verification' && failedJob.conclusion === 'failure', 'S02 two-finding correction live failed job mismatch');
    const failedSteps = (failedJob.steps ?? []).filter(step => step.name === s02TwoFindingCorrection.failedWorkflow.failedStep);
    assert(failedSteps.length === 1 && failedSteps[0].conclusion === 'failure', 'S02 two-finding correction live failed step mismatch');
    assert(failedArtifact.name === s02TwoFindingCorrection.failedWorkflow.artifactName && failedArtifact.digest === s02TwoFindingCorrection.failedWorkflow.artifactDigest && failedArtifact.expired === false && failedArtifact.workflow_run?.head_sha === s02TwoFindingCorrection.entry.head, 'S02 two-finding correction live failure artifact mismatch');
  }
}
// END_S02_TRUSTED_HARNESS_CORRECTION_ROUND_009

// BEGIN_S02_TRUSTED_HARNESS_CORRECTION_ROUND_010
if (s02RemainingTwoCorrection) {
  assertExactKeySet(s02RemainingTwoCorrection, ['schemaVersion', 'artifactId', 'createdAt', 'repository', 'branch', 'changeControl', 'entry', 'authorityWorkflow', 'failedWorkflow', 'diagnosis', 'correction', 'boundaries', 'status'], 'S02 remaining-two correction');
  assertExactKeySet(s02RemainingTwoCorrection.entry, ['head', 'tree'], 'S02 remaining-two correction entry');
  assertExactKeySet(s02RemainingTwoCorrection.authorityWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 remaining-two authority workflow');
  assertExactKeySet(s02RemainingTwoCorrection.failedWorkflow, ['commit', 'tree', 'runId', 'runAttempt', 'jobId', 'conclusion', 'failedStep', 'summary', 'artifactId', 'artifactName', 'artifactDigest'], 'S02 remaining-two failed workflow');
  assertExactKeySet(s02RemainingTwoCorrection.diagnosis, ['type', 'failedAssertions', 'closedFindings', 'visualFindings', 'measurementFindings', 'allFifteenExactPngsPreserved', 'originalFailureArtifactPreserved'], 'S02 remaining-two diagnosis');
  assertExactKeySet(s02RemainingTwoCorrection.correction, ['rule', 'changedPaths', 'stylesBeforeSha256', 'stylesAfterSha256', 'stylesPatchSha256', 'manifestBeforeSha256', 'manifestAfterSha256', 'manifestPatchSha256', 'verifierAfterSha256', 'verifierPatchSha256', 'acceptanceThresholdsChanged', 'qualityCoverageWeakened', 'browserCollectorChanged', 'browserQaChanged', 'fontSizeReduced', 'failureEvidenceUploadRetained'], 'S02 remaining-two exact correction');
  assertExactKeySet(s02RemainingTwoCorrection.boundaries, ['rootRuntimeChanged', 'gameCoreChanged', 'gameDataChanged', 'economyChanged', 'saveSchemaChanged', 'backendChanged', 'productionChanged', 'productionAliasChanged', 'physicalIPhoneVerified', 'step4Pass', 'step5Allowed'], 'S02 remaining-two correction boundaries');
  assert(s02RemainingTwoCorrection.schemaVersion === 1 && s02RemainingTwoCorrection.artifactId === 'cats-tower-s02-golden-master-p1-trusted-harness-correction-round-010' && s02RemainingTwoCorrection.createdAt === '2026-09-02', 'S02 remaining-two correction identity/date mismatch');
  assert(s02RemainingTwoCorrection.repository === '2hg7trp7rv-design/cats_tower' && s02RemainingTwoCorrection.branch === 'kimi' && s02RemainingTwoCorrection.changeControl === s02RepairControlPath, 'S02 remaining-two correction authority mismatch');
  assert(JSON.stringify(s02RemainingTwoCorrection.entry) === JSON.stringify({ head: 'c988585130aba9872cbe61bde97d5abca1baa176', tree: '3f85a54f8a3ae50930d3348d38df2bf31deb4239' }), 'S02 remaining-two correction entry is not the exact failed round-009 correction commit/tree');
  assert(JSON.stringify(s02RemainingTwoCorrection.authorityWorkflow) === JSON.stringify({
    commit: s02RemainingTwoCorrection.entry.head,
    tree: s02RemainingTwoCorrection.entry.tree,
    runId: 33599003346,
    runAttempt: 1,
    jobId: 100148484561,
    conclusion: 'SUCCESS',
    artifactId: 9834462333,
    artifactName: 'phase0-current-governance-c988585130aba9872cbe61bde97d5abca1baa176-33599003346-1',
    artifactDigest: 'sha256:acdbe55afb633a44c7a7950a896de753e5452c40840b7f7a95ca9a6eaa494ade'
  }), 'S02 remaining-two correction does not bind the successful exact-entry authority workflow');
  assert(JSON.stringify(s02RemainingTwoCorrection.failedWorkflow) === JSON.stringify({
    commit: s02RemainingTwoCorrection.entry.head,
    tree: s02RemainingTwoCorrection.entry.tree,
    runId: 33599003348,
    runAttempt: 1,
    ...
    failedStep: 'Capture and verify eight masters plus required responsive variants',
    summary: 'FAIL_S02_GOLDEN_MASTER_P1_BROWSER: 15 scenarios, 2 failures',
    artifactId: 9834454926,
    artifactName: 'step4-s02-golden-master-p1-semantic-c988585130aba9872cbe61bde97d5abca1baa176-33599003348-1',
    artifactDigest: 'sha256:83f96fe808db637bcf96d85f9c6bdf78c69717f89ad8823ca5702598057e7f27'
  }), 'S02 remaining-two correction does not bind the exact failed workflow/job/summary/artifact');
  assert(JSON.stringify(s02RemainingTwoCorrection.diagnosis) === JSON.stringify({
    type: 'ROUND_009_CLOSED_COMPACT_OFFLINE_COLOR_AND_HEADER_OVERLAP_BUT_EXPOSED_TEXT200_ACTION_FIT_AND_MODAL_PALETTE_REGRESSION',
    failedAssertions: [
      'GM07C320TEXT200: screenshot pixel complexity/opacity gate failed',
      'TEXT_200_PERCENT_NO_LOSS: trusted recomputation failed'
    ],
    closedFindings: [
      'GM07C320 reached 262 quantized colors and passed the unchanged 256 threshold.',
      'All nine critical labels retained exact 200 percent font scaling, fit their scroll boxes and no longer overlapped.'
    ],
    visualFindings: [
      'GM07C320TEXT200 reached 231 quantized colors because the enlarged parchment content hid most of the lower material inlay; the visible clock seal needed its own restrained enamel palette.'
    ],
    measurementFindings: [
      'The 200 percent primary action reported scrollHeight 102 CSS pixels against clientHeight 98 CSS pixels in both zero-safe-area and nonzero-safe-area captures; every other recomputed text-scale predicate passed.'
    ],
    allFifteenExactPngsPreserved: true,
    originalFailureArtifactPreserved: true
  }), 'S02 remaining-two diagnosis differs from the preserved fifteen-capture failed run and artifact');
  assert(s02RemainingTwoCorrection.correction.rule === 'ADD_A_RESTRAINED_BRASS_PATINA_ENAMEL_RING_TO_THE_VISIBLE_OFFLINE_SEAL_AND_EXPAND_THE_200_PERCENT_PRIMARY_ACTION_BY_EIGHT_PIXELS_WITHOUT_SHRINKING_TEXT_OR_LOWERING_THRESHOLDS' && JSON.stringify(s02RemainingTwoCorrection.correction.changedPaths) === JSON.stringify(s02RemainingTwoCorrectionChangedPaths), 'S02 remaining-two correction rule/path set mismatch');
  assert(s02RemainingTwoCorrection.correction.stylesBeforeSha256 === '666c48708f67d6e2b24d1861bae8bbf1778a4d5a34b3fd5d81a74bc429c8699b' && s02RemainingTwoCorrection.correction.stylesAfterSha256 === 'f57a5d34f3f07a31f72c05ef6c31d698bbda9c5a15deefcea1e8c587a8de3d25' && s02RemainingTwoCorrection.correction.stylesPatchSha256 === 'sha256:dabc4c04d8ac67c7c37af6fdd75e69bcbb260f36162905374e3ae364857da0a6', 'S02 remaining-two style digest/patch mismatch');
  assert(s02RemainingTwoCorrection.correction.manifestBeforeSha256 === '2307d9c35cfcff06d239bcbc3b3870b78ad1feff7d6afcf30c816001a46e3625' && s02RemainingTwoCorrection.correction.manifestAfterSha256 === 'f308ce486a4ffec981d25423f73cdda2ccb46db64a72f5e51bf8deb9f675a8da' && s02RemainingTwoCorrection.correction.manifestPatchSha256 === 'sha256:01cff4464a23fc597a3aa985591094a324b0a9d2c047ae5e3af2fe60727da085', 'S02 remaining-two manifest digest/patch mismatch');
  assert(s02RemainingTwoCorrection.correction.acceptanceThresholdsChanged === false && s02RemainingTwoCorrection.correction.qualityCoverageWeakened === false && s02RemainingTwoCorrection.correction.browserCollectorChanged === false && s02RemainingTwoCorrection.correction.browserQaChanged === false && s02RemainingTwoCorrection.correction.fontSizeReduced === false && s02RemainingTwoCorrection.correction.failureEvidenceUploadRetained === true, 'S02 remaining-two correction weakens coverage, changes evidence collection or shrinks text');
  assert(JSON.stringify(s02RemainingTwoCorrection.boundaries) === JSON.stringify({ rootRuntimeChanged: false, gameCoreChanged: false, gameDataChanged: false, economyChanged: false, saveSchemaChanged: false, backendChanged: false, productionChanged: false, productionAliasChanged: false, physicalIPhoneVerified: false, step4Pass: false, step5Allowed: false }), 'S02 remaining-two correction crosses a protected product/release boundary');
  assert(s02RemainingTwoCorrection.status === 'CORRECTED_REMAINING_TWO_BROWSER_ASSERTIONS_PENDING_RERUN', 'S02 remaining-two correction status mismatch');
  assert(s02RemainingTwoCorrectionCommit && git(['rev-parse', `${s02RemainingTwoCorrectionCommit}^`]) === s02RemainingTwoCorrection.entry.head, 'S02 remaining-two correction is not the immediate child of the exact failed commit');
  assert(git(['rev-parse', `${s02RemainingTwoCorrection.entry.head}^{tree}`]) === s02RemainingTwoCorrection.entry.tree, 'S02 remaining-two correction entry tree mismatch');
  assertExactChangedPaths(s02RemainingTwoCorrection.entry.head, s02RemainingTwoCorrectionCommit, s02RemainingTwoCorrectionChangedPaths, 'S02 remaining-two correction commit');
  assertAddedOnceAndUnchanged(s02RemainingTwoCorrectionPath, s02RemainingTwoCorrectionCommit);

  const priorRemainingTwoStylesSource = textAt(s02RemainingTwoCorrection.entry.head, 'step4/s02/golden-master-p1/styles.css');
  const correctedRemainingTwoStylesSource = textAt(s02RemainingTwoCorrectionCommit, 'step4/s02/golden-master-p1/styles.css');
  const remainingTwoStylesPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02RemainingTwoCorrection.entry.head, s02RemainingTwoCorrectionCommit, '--', 'step4/s02/golden-master-p1/styles.css'], { cwd: root, encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
  assert(sha256Text(priorRemainingTwoStylesSource) === s02RemainingTwoCorrection.correction.stylesBeforeSha256 && sha256Text(correctedRemainingTwoStylesSource) === s02RemainingTwoCorrection.correction.stylesAfterSha256 && `sha256:${sha256Text(remainingTwoStylesPatch)}` === s02RemainingTwoCorrection.correction.stylesPatchSha256, 'S02 remaining-two style bytes/patch differ from the immutable correction record');
  assert(correctedRemainingTwoStylesSource.includes('radial-gradient(circle, #ddb15c 0 54%, transparent 56%)') && correctedRemainingTwoStylesSource.includes('conic-gradient(from -12deg, #75402a 0 14%, #b96d51 14% 28%, #527f77 28% 42%, #e5bd64 42% 58%, #6c537d 58% 72%, #4f8278 72% 86%, #75402a 86% 100%)') && correctedRemainingTwoStylesSource.includes('height: 108px;'), 'S02 remaining-two correction lacks the restrained seal material or 200-percent action expansion');
  assert(priorRemainingTwoStylesSource.split('font-size:').length === correctedRemainingTwoStylesSource.split('font-size:').length, 'S02 remaining-two correction shrinks or adds typography instead of repairing material and bounds');

  const priorRemainingTwoManifestSource = textAt(s02RemainingTwoCorrection.entry.head, 'step4/s02/golden-master-p1/review-manifest.json');
  const correctedRemainingTwoManifestSource = textAt(s02RemainingTwoCorrectionCommit, 'step4/s02/golden-master-p1/review-manifest.json');
  const remainingTwoManifestPatch = execFileSync('git', ['diff', '--no-ext-diff', '--no-color', s02RemainingTwoCorrection.entry.head, s02RemainingTwoCorrectionCommit, '--', 'step4/s02/golden-master-p1/review-manifest.json'], { cwd: root, encoding: 'utf8', maxBuffer: 4 * 1024 * 1024 });
  assert(sha256Text(priorRemainingTwoManifestSource) === s02RemainingTwoCorrection.correction.manifestBeforeSha256 && sha256Text(correctedRemainingTwoManifestSource) === s02RemainingTwoCorrection.correction.manifestAfterSha256 && `sha256:${sha256Text(remainingTwoManifestPatch)}` === s02RemainingTwoCorrection.correction.manifestPatchSha256, 'S02 remaining-two manifest bytes/patch differ from the immutable correction record');
  const correctedRemainingTwoManifest = JSON.parse(correctedRemainingTwoManifestSource);
  const correctedRemainingTwoStylesManifestEntry = correctedRemainingTwoManifest.routeFiles?.find(entry => entry.path === 'step4/s02/golden-master-p1/styles.css');
  assert(JSON.stringify(correctedRemainingTwoStylesManifestEntry) === JSON.stringify({ path: 'step4/s02/golden-master-p1/styles.css', bytes: 53161, sha256: s02RemainingTwoCorrection.correction.stylesAfterSha256, gitBlob: '670d7aa6bc621853994fd7db30db9a17ef238bbf' }), 'S02 remaining-two manifest does not bind the corrected stylesheet bytes/hash/blob');

  const correctedRemainingTwoVerifierSource = textAt(s02RemainingTwoCorrectionCommit, 'tests/governance/verify-current-authority.mjs');
  ...
  if (requireLiveActions) {
    ...
    const authorityArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02RemainingTwoCorrection.authorityWorkflow.artifactId}`);
    assert(authorityRun.head_sha === s02RemainingTwoCorrection.entry.head && authorityRun.head_branch === 'kimi' && authorityRun.status === 'completed' && authorityRun.conclusion === 'success' && authorityRun.run_attempt === 1, 'S02 remaining-two correction live authority run mismatch');
    assert(authorityJob.run_id === authorityRun.id && authorityJob.head_sha === s02RemainingTwoCorrection.entry.head && authorityJob.name === 'current-authority' && authorityJob.conclusion === 'success', 'S02 remaining-two correction live authority job mismatch');
    assert(authorityArtifact.name === s02RemainingTwoCorrection.authorityWorkflow.artifactName && authorityArtifact.digest === s02RemainingTwoCorrection.authorityWorkflow.artifactDigest && authorityArtifact.expired === false && authorityArtifact.workflow_run?.head_sha === s02RemainingTwoCorrection.entry.head, 'S02 remaining-two correction live authority artifact mismatch');
    const failedRun = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/runs/${s02RemainingTwoCorrection.failedWorkflow.runId}`);
    const failedJob = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/jobs/${s02RemainingTwoCorrection.failedWorkflow.jobId}`);
    const failedArtifact = ghJson(`/repos/2hg7trp7rv-design/cats_tower/actions/artifacts/${s02RemainingTwoCorrection.failedWorkflow.artifactId}`);
    assert(failedRun.head_sha === s02RemainingTwoCorrection.entry.head && failedRun.head_branch === 'kimi' && failedRun.status === 'completed' && failedRun.conclusion === 'failure' && failedRun.run_attempt === 1, 'S02 remaining-two correction live failed run mismatch');
    assert(failedJob.run_id === failedRun.id && failedJob.head_sha === s02RemainingTwoCorrection.entry.head && failedJob.name === 'Static, eight-master responsive and accessibility verification' && failedJob.conclusion === 'failure', 'S02 remaining-two correction live failed job mismatch');
    const failedSteps = (failedJob.steps ?? []).filter(step => step.name === s02RemainingTwoCorrection.failedWorkflow.failedStep);
    assert(failedSteps.length === 1 && failedSteps[0].conclusion === 'failure', 'S02 remaining-two correction live failed step mismatch');
    assert(failedArtifact.name === s02RemainingTwoCorrection.failedWorkflow.artifactName && failedArtifact.digest === s02RemainingTwoCorrection.failedWorkflow.artifactDigest && failedArtifact.expired === false && failedArtifact.workflow_run?.head_sha === s02RemainingTwoCorrection.entry.head, 'S02 remaining-two correction live failure artifact mismatch');
  }
}
