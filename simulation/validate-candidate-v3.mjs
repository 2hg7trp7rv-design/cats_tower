...
const dataPath = resolve(process.argv[2] ?? 'simulation/candidate-v3.json');
const schemaPath = resolve('simulation/candidate-v3.schema.json');
try {
  ...
  console.log(JSON.stringify({ ok: true, artifactId: 'cats-tower-candidate-v3-validator' }));
} catch (error) {
