...
const unsigned = (value, path, { positive = false } = {}) => {
  ...
  if (positive) check(n > 0n, 'RUN_PLAN_POSITIVE', `${path} must be positive`);
  return n;
};

const top = ['schemaVersion','planId','candidateId','algorithmVersion','roundingVersion','step2Mode','fullExecutionOwner','builds','personas','seeds','scenarioCount','horizons','qualificationHorizons','highVolumeSuites','holdoutPolicy','output'];
exactKeys(plan, top, '#');
check(plan.schemaVersion === '2.0.0', 'RUN_PLAN_SCHEMA_VERSION', 'schemaVersion must be 2.0.0');
...
check(plan.algorithmVersion === 'cats-tower-simulator-v2.0.0', 'RUN_PLAN_ALGORITHM', 'algorithmVersion mismatch');
check(plan.roundingVersion === 'ct-rational-half-even-v1', 'RUN_PLAN_ROUNDING', 'roundingVersion mismatch');
check(plan.step2Mode === 'QUALIFICATION_ONLY', 'RUN_PLAN_STEP2_SCOPE', 'Step 2 must be qualification only');
...
exactKeys(plan.scenarioCount, ['fullGameplayMatrix','qualification'], '#/scenarioCount');
...
check(eq(plan.qualificationHorizons, expectedHorizons.map((x)=>x.id)), 'RUN_PLAN_QUALIFICATION_HORIZONS', 'qualification horizon order or coverage mismatch');
...
exactKeys(plan.output, ['qualificationPath','schemaPath','canonicalJson','hashEachScenario'], '#/output');
check(plan.output?.qualificationPath === 'quality-reviews/step-2-executable-contract-v2/qualification-result.json', 'RUN_PLAN_OUTPUT_PATH', 'qualification output path mismatch');
check(plan.output?.schemaPath === 'simulation/result-v2.schema.json', 'RUN_PLAN_RESULT_SCHEMA', 'result schema path mismatch');
...
console.log(JSON.stringify({ok:true,runPlan:planPath,fullScenarioCount:fullCount.toString(),qualificationScenarioCount:qualificationCount.toString(),sampledSeedCount:String(sampledSeeds.size),step2Mode:plan.step2Mode,fullExecutionOwner:plan.fullExecutionOwner}));
