...
(function () {
  ...
  /* =====================================================================
   * WebAudio 簡易シンセ (招集/命中/コイン/制圧/夜明け)
   * =================================================================== */
  const Audio2 = {
  ...
  function drawUnits(t) {
    ...
    if (g) {
      ...
      if (g.boss) { grd.addColorStop(0, '#c04ae0'); grd.addColorStop(1, '#e05a4a'); }
      else { grd.addColorStop(0, '#e05a4a'); grd.addColorStop(1, '#e09a3a'); }
      ctx.fillStyle = grd;
      ctx.fillRect(x, y, w * r, 12);
      ctx.font = 'bold 10px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillStyle = '#fff';
      const label = (g.boss ? '👑 BOSS ' : '') + game.floor + 'F 守護者  ' + fmt(g.hp) + ' / ' + fmt(g.maxHp);
      ctx.fillText(label, view.w / 2, y + 10);
      ctx.restore();
    }
  }
  ...
  function processEvents(events) {
    for (const ev of events) {
      switch (ev.type) {
        ...
        case 'dawn':
          Audio2.dawn();
          showBanner('🌅 夜明け — ✨' + ev.gain + ' を得た', 2.2, true);
          fx.flashT = 2;
          break;
        case 'shop-built':
          Audio2.build();
   ...
   * =================================================================== */
  function updateHud() {
    el('hud-floor').textContent = game.floor + 'F' + (isBossFloor(game.floor) ? ' 👑' : '');
    ...
    dawn.title = ok ? '✨' + game.prestigeGain() + ' を得られる' : '10F制圧で解放';
  ...
  function openAgency() {
    ...
    const sheet = openSheet('<h3>🐾 ねこ派遣所</h3><p class="sheet-sub">合計Lv ' + totalLv + ' — 雇用した猫は恒久的に編成へ。Lvで攻撃力+25%。</p>' + rows +
  ...
  function openForge() {
    ...
    for (let k = 1; k <= total + 2; k++) {
      ...
      if (owned) {
        rows.push('<div class="buy-row"><span class="ricon">' + w.icon + '</span>' +
          '<div class="rinfo"><b>' + w.name + '</b><p>全攻撃 ×' + w.mult + ' (所持中)</p></div>' +
          '<span>✅</span></div>');
      } else if (next) {
        rows.push('<div class="buy-row"><span class="ricon">' + w.icon + '</span>' +
          '<div class="rinfo"><b>' + w.name + '</b><p>全攻撃 ×' + w.mult + ' — 所持武器はすべて乗算で効く</p></div>' +
    ...
    const sheet = openSheet('<h3>⚒️ かじ屋</h3><p class="sheet-sub">現在の武器倍率: ×' + fmt(game.weaponMult()) + '</p>' + rows.join('') +
      '<button class="ghost close">閉じる</button>');
  ...
  function openItemShop() {
    ...
    const sheet = openSheet('<h3>🎒 道具屋</h3><p class="sheet-sub">%バフは繰り返し購入可。コストは逓増。</p>' + rows +
      '<button class="ghost close">閉じる</button>');
  ...
  function openPendingShopChoice() {
    ...
    const cards = ch.options.map(id => {
      ...
      const same = Object.values(game.shopsBuilt).filter(x => x === id).length;
      return '<div class="card"><div class="card-head"><span class="sicon">' + s.icon + '</span><b>' + s.name + '</b>' +
        (same > 0 ? '<small>(' + (same + 1) + '店目 効果×' + (same === 1 ? 1.5 : 2) + ')</small>' : '') + '</div>' +
        '<p>' + s.desc + (id === 'diner' ? ' → +' + fmt(DATA.dinerIncome(ch.floor)) + '/秒' : '') + '</p>' +
        '<button class="primary pick-shop" data-id="' + id + '">ここに建てる</button></div>';
    }).join('');
    const sheet = openSheet('<h3>🏪 ' + ch.floor + 'F に店を建てる</h3>' +
      '<p class="sheet-sub">制圧した階が店舗になる。あとから塔リストで建て替えもできる。</p>' +
  ...
  function openTower() {
    ...
    const sheet = openSheet('<h3>🗼 塔 — 制圧階リスト</h3>' +
      '<p class="sheet-sub">制圧済み ' + game.maxFloor + 'F / 店はタップで建て替え自由。魚食堂=収益, 武器蔵=攻撃, 派遣支所=出撃速度, 金貨倉庫=オフライン。</p>' +
  ...
  function renderPrestigeModal() {
    ...
    el('prestige-gain').textContent = game.prestigeAvailable
      ? '✨ +' + gain + ' きらめき (到達 ' + game.maxFloor + 'F / ボス撃破 ' + game.bossKills + ')'
